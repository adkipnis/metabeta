...
Usage (from repo root):
    uv run python experiments/analytical/statsmodels_reml.py [--n_datasets 1000] [--seed 0]
"""
...
def run_comparison(
    ...
    data_dir = DATA_DIR / data_cfg['data_id']
    ...
    rows = [
        [
            'FFX (β)',
            f'{_nrmse(beta_err_our, beta_true_flat):.4f}',
            f'{_bias(beta_err_our):+.4f}',
            f'{_nrmse(beta_err_reml, beta_true_flat):.4f}',
            f'{_bias(beta_err_reml):+.4f}',
        ],
        [
            'RFX (BLUPs)',
            f'{_nrmse(rfx_err_our, rfx_true_flat):.4f}',
            f'{_bias(rfx_err_our):+.4f}',
            f'{_nrmse(rfx_err_reml, rfx_true_flat):.4f}',
            f'{_bias(rfx_err_reml):+.4f}',
        ],
        [
            'Sigma(RFX)',
            f'{_nrmse(srfx_our, srfx_true_arr):.4f}',
            f'{_bias(srfx_our):+.4f}',
            f'{_nrmse(srfx_reml, srfx_true_arr):.4f}',
            f'{_bias(srfx_reml):+.4f}',
        ],
        [
            'Sigma(Eps)',
            f'{_nrmse(seps_our, seps_true_arr):.4f}',
            f'{_bias(seps_our):+.4f}',
            f'{_nrmse(seps_reml, seps_true_arr):.4f}',
            f'{_bias(seps_reml):+.4f}',
        ],
    ]
    print(
        tabulate(
    ...
    # -----------------------------------------------------------------------
    def col(k):
        return np.array([r[k] for r in meta])
    ...
    for i in range(len(edges) - 1):
        ...
        rows.append(
            [
                ...
                int(mask.sum()),
                f'{_bias(eo):+.4f}',
                f'{np.sqrt(np.mean(eo**2)):.4f}',
                f'{_bias(er):+.4f}',
                f'{np.sqrt(np.mean(er**2)):.4f}',
            ]
        )
    print(
        tabulate(
    ...
    print(
        tabulate(
            rows,
            headers=['bg_df', 'N', 'Bias (ours)', 'RMSE (ours)', 'Bias (REML)', 'RMSE (REML)'],
            tablefmt='simple',
        )
    )
    ...
    print(
        tabulate(
            rows,
            headers=['q', 'N', 'NRMSE (ours)', 'Bias (ours)', 'NRMSE (REML)', 'Bias (REML)'],
            tablefmt='simple',
        )
    )
    ...
    def col(k):
        return np.array([r[k] for r in meta])  # noqa: redefined for convenience
    ...
    for i in range(len(edges_nd) - 1):
        ...
        rows.append(
            [
                f'{lo:.1f}–{hi:.1f}',
                int(mask.sum()),
                f'{_nrmse(eo_flat, tr_flat):.4f}',
                f'{_bias(eo_flat):+.4f}',
                f'{_nrmse(er_flat, tr_flat):.4f}',
                f'{_bias(er_flat):+.4f}',
            ]
        )
    print(
    ...
    for lo, hi in zip(ng_edges[:-1], ng_edges[1:]):
        ...
        rows.append(
            [
                f'{lo}–{hi-1}',
                int(mask.sum()),
                f'{_nrmse(eo_all[mask], tr_all_r[mask]):.4f}',
                f'{_bias(eo_all[mask]):+.4f}',
                f'{_nrmse(er_all[mask], tr_all_r[mask]):.4f}',
                f'{_bias(er_all[mask]):+.4f}',
            ]
        )
    print(
        tabulate(
