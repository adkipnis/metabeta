...
METHODS = ['normal_eb', 'normal_beta_grid', 'current']
TAIL_METRICS = [
    'ffx_eb_rmse',
    'sigma_eb_rmse',
    'blup_eb_rmse',
    ...
    for data_id in args.data_ids:
        ...
        for path in _pathsForArgs(cfg, args.partition, args.n_epochs):
            ...
            for batch in Dataloader(path, batch_size=args.batch_size, shuffle=False):
                ...
                B = batch['X'].shape[0]
                stats = _normalStats(batch, batch['Z'][..., :max_q], args)
        ...
        for candidate in selected:
            ...
            for diag in candidate['diags']:
                ...
                diag['blup_inla_rmse'] = _rmse(
                    (est['blups'][:m] - candidate['rfx_true']).reshape(-1)
                )
                ...
                diag['sigma_inla_bias_mean'] = float(
                    np.mean(est['sigma_rfx'] - candidate['srfx_true'])
                )
