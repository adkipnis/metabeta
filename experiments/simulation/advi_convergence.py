...
Usage (from repo root):
    uv run python experiments/simulation/advi_convergence.py
    uv run python experiments/simulation/advi_convergence.py --n_datasets 8 --viter 100000
"""
...
def build_model(ds: dict) -> pm.Model:
    ...
    with pm.Model() as model:
        betas = [
            pm.Normal('Intercept' if j == 0 else f'x{j}', mu=nu_ffx[j], sigma=tau_ffx[j])
            for j in range(d)
        ]
        if correlated:
...
def run(args: argparse.Namespace) -> None:
    ...
    print(
        f'Configs: {len(ADVI_CONFIGS)} ADVI + {"Pathfinder" if fit_pathfinder else "no Pathfinder"}'
    )
    print()
    ...
    gs = GridSpec(n_ds + 1, 2, figure=fig, height_ratios=[3] * n_ds + [2], hspace=0.55, wspace=0.35)
    ...
    for row_i, row in enumerate(all_results):
        ...
        colors_bar = [
            '#2ca02c' if p < 0.005 and not r else '#ff7f0e' if p < 0.02 else '#d62728'
        ...
        ax_heat.axvline(
            0.005, color='green', linestyle=':', linewidth=1, alpha=0.6, label='converged (<0.005)'
        )
        ax_heat.axvline(
            0.020, color='orange', linestyle=':', linewidth=1, alpha=0.6, label='marginal (<0.02)'
        )
    ...
    if any(row['pathfinder'] and row['pathfinder']['ok'] for row in all_results):
        ...
        ax_time.bar(
            x + len(ADVI_CONFIGS) / 2 * width + width,
            pf_walls,
            width,
            label='pathfinder',
            color='#9467bd',
    ...
    ax_time.set_xticklabels(
        [f'ds={r["idx"]}\nn={r["n"]},q={r["q"]}' for r in all_results], fontsize=7
    )
    ...
    col_w = 10
    header = f'{"ds":>3}  {"n":>5}  {"q":>2}  ' + '  '.join(
        f'{c[0]:>{col_w}}' for c in ADVI_CONFIGS
    )
    if fit_pathfinder:
    ...
    if fit_pathfinder:
        ...
        print(
            f'  Pathfinder:    {pf_ok}/{len(all_results)} OK, mean wall={pf_mean:.1f}s '
