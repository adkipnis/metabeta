...
$$\tilde{\mathbf{s}} = [\mathbf{s}_i]_{i=1, \ldots, m} \in \mathbb R^{m \times h_l}$$
$$\mathbf{s} = f_{\Sigma_g}(\tilde{\mathbf{s}}) \in \mathbb R^{1 \times h_g}$$
...
$$\tilde{\mathbf H}^{(0)} = \begin{pmatrix} \mathbf c \\\ \mathbf H^{(0)}\end{pmatrix} \in \mathbb{R}^{(n+1) \times d_{\mathrm{model}}}$$
