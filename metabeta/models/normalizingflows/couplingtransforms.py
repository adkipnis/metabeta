...
class RationalQuadratic(CouplingTransform):
    ...
    def __init__(
        ...
        net_type = net_kwargs.get("net_type", "mlp")
        ...
        self.tail_constant = np.log(np.exp(1.0 - self.min_derivative) - 1.0)
    ...
    def propose(self, x1, condition=None):
        ...
        return dict(
            raw_widths=raw_widths, raw_heights=raw_heights, raw_derivs=raw_derivs
        )
    ...
    def _constrain(self, raw_widths, raw_heights, raw_derivatives):
        ...
        widths = (
            self.min_bin_width + (1 - self.min_bin_width * self.num_bins) * widths
        )  # ensure min
        cumwidths = torch.cumsum(widths, dim=-1)
        cumwidths = F.pad(cumwidths, pad=(1, 0), mode="constant", value=0.0)  # leftpad
        cumwidths = left + cumwidths * (
            right - left
        )  # stretch cumulative widths to [-B, B]
        ...
        heights = (
            self.min_bin_height + (1 - self.min_bin_height * self.num_bins) * heights
        )
    ...
    def __call__(self, input2, params, mask2=None, inverse=False):
        # extract params
        raw_widths, raw_heights, raw_derivs = (
            params["raw_widths"],
            params["raw_heights"],
            params["raw_derivs"],
        )
        # masks
        inside_interval_mask = (input2 >= -self.tail_bound) & (
            input2 <= self.tail_bound
        )
