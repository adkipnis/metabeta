...
def setup() -> argparse.Namespace:
    ...
    parser.add_argument('--partition', type=str, default='test', choices=['train', 'test', 'valid'],
        help='Which partition to fit: train, test, or valid (default=test)')
    parser.add_argument('--epoch', type=int, default=None,
        help='Epoch number (required when --partition train)')
    return setupConfigParser(parser, generateSimulationConfig, 'Fit hierarchical datasets with PyMC (NUTS or ADVI).')
# fmt: on
...
if __name__ == '__main__':
    ...
    for _k, _v in [
        ...
        if not hasattr(cfg, _k):
            setattr(cfg, _k, _v)
    if getattr(cfg, 'partition', 'test') == 'train' and getattr(cfg, 'epoch', None) is None:
